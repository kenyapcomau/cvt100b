...
void *aealloc(void *blk,unsigned size)
 {
 void *rt;
 if(blk==NULL) return aalloc(size);
 if(size) rt=realloc(blk,size);
 else rt=realloc(blk,1);
 if(rt==NULL)
  {
  fprintf(stderr,"Out of memory\n");
  exit(1);
...
unsigned long hash(unsigned char *name)
 ...
 while(*name) accu=accu*34857+*name++;
 return accu;
 }
...
struct label
 ...
 /* Group label is relatice to: must be zero if 'seg' is zero */
 /* What is this for? */
...
struct segment
 ...
 unsigned pos;		/* Current position withing segment */
 unsigned inc;		/* Increment to get next position */
...
struct label *labelfind(char *name)
 ...
 struct label *l;
 for(l=labels[hval];l;l=l->next) if(!strcmp(name,l->name)) return l;
...
struct segment *segmentfind(char *name)
 ...
 struct segment *l;
 for(l=segments[hval];l;l=l->next) if(!strcmp(name,l->name)) return l;
...
struct segment *privatefind(char *name)
 ...
 c2=name[y]; name[y]=0;
 for(z=0;z!=nmodules;++z) if(!strcmp(modules[z]->name,name+x)) break;
 if(z!=nmodules) seg=private1find(name,modules[z]);
 name[x-1]=c1;
 name[y]=c2;
 return seg;
 }
...
unsigned iteratedsz(unsigned char *pgm,unsigned *x,int n)
 ...
 while(n--)
  ...
  else amnt+=iteratedsz(pgm,x,blk);
  }
 return amnt;
 }
...
void iterated(unsigned char *pgm,unsigned *x,int n,unsigned char **ptr)
 ...
 while(n--)
  ...
  if(!blk)
   {
   while(rpt--) memcpy(*ptr,pgm+*x+1,pgm[*x]), *ptr+=pgm[*x];
   *x+=pgm[*x]+1;
   }
  else iterated(pgm,x,blk,ptr);
  }
 }
...
void pass1(FILE *fd,char *objname)
 ...
 if(nxt>pgmbksiz) pgm=(unsigned char *)aealloc(pgm,pgmbksiz=nxt);
 fread(pgm,1,nxt,fd);
 --nxt;
 x=0;
 ...
 case 0xA0:			/* Code/Data (only record start & size) */
  ...
  sz-=x-ox;
  if(ofst+seg->length<seg->start) seg->start=ofst+seg->length;
  if(ofst+sz+seg->length>seg->end) seg->end=ofst+sz+seg->length;
  }
  break;
 ...
 case 0x8C:			/* External names */
  for(;x!=nxt;x+=pgm[x]+1, IDXINC(x))
   ...
   if(!label->ext) label->ext=1;
   if(listfile!=NULL)
    {
    if(label->bksiz==label->nref)
     label->ref=(struct module **)aealloc(label->ref,(label->bksiz+=20)*sizeof(struct module *));
 ...
 case 0x9A:			/* Group definition */
  ...
  if(grp->seglst!=NULL)
   ...
   for(y=0;x!=nxt;++y)
    ...
    for(z=0;z!=grp->lstsiz;++z) if(grp->seglst[z]==seg) goto over;
    grp->seglst=(struct segment **)aealloc(grp->seglst,sizeof(struct segment *)*++grp->lstsiz);
    grp->seglst[z]=seg;
    over:;
    }
   }
   ...
   {
   unsigned ox=x;
   for(y=0;x!=nxt;++x, IDXINC(x)) ++y;
   grp->lstsiz=y;
   grp->seglst=(struct segment **)acalloc(y,sizeof(struct segment *));
   for(y=0, x=ox;x!=nxt;) grp->seglst[y++]=segarray[++x, IDXINC(x)-1];
   grp->gmodule=module;
   }
 ...
 case 0x98:			/* Segment definition */
  ...
  align=(pgm[x]>>5);
  combine=(7&(pgm[x]>>2));
  if(pgm[x]&2) len=0x10000;
  ++x;
  if(!align) frame=SIZINC(x), ofst=pgm[x++];
  if(!len) len=SIZ(x);
  x+=2;
  nidx=IDXINC(x);
  cidx=IDXINC(x);
  ...
  if(!combine)
   ...
   for(z=0;z!=class->csiz;++z) if(seg==class->clst[z]) goto ovr;
   ...
   segarray[segidx++]=seg;
   if(seg->module!=NULL)
    { /* Segment already defined.  Check for mismatch */
    if(align>seg->alignment && seg->alignment)
     seg->alignment=align;
    if(combine!=seg->combine)
     fprintf(stderr,"Segment mismatch (combine) %s(%s) %s\n",seg->name,module->name,seg->module->name), errval=1;
    if(frame!=seg->frame)
     fprintf(stderr,"Segment mismatch (at) %s(%s) %s\n",seg->name,module->name,seg->module->name), errval=1;
    if(seg->combine==6)
     { if(len>seg->length) seg->length=len; }
    else if(len)
     if(!seg->nlength)
      switch(align)
       {
      case 2: seg->length=((seg->length+1)&~1); break;
      case 3: seg->length=((seg->length+15)&~15); break;
      case 4: seg->length=((seg->length+255)&~255); break;
       }
     else
      switch(align)
       {
      case 2: seg->nlength=((seg->nlength+1)&~1); break;
      case 3: seg->nlength=((seg->nlength+15)&~15); break;
      case 4: seg->nlength=((seg->nlength+255)&~255); break;
       }
    seg->nlength+=len;
    seg->ncontrib++;
    }
    ...
    seg->ncontrib++;
    class=namearray[cidx-1];
    for(z=0;z!=class->csiz;++z) if(seg==class->clst[z]) goto ovr1;
    class->clst=(struct segment **)aealloc(class->clst,++class->csiz*sizeof(struct segment *));
    class->clst[z]=seg;
    ovr1:;
    }
 ...
 case 0x90:			/* Public names */
  ...
  while(x!=nxt)
   ...
   x+=pgm[x]+1;
   label=labelfind(buf);
   if(label->module!=NULL) fprintf(stderr,"Multiply defined symbol: %s(%s) (%s)\n",label->name,module->name,label->module->name), errval=1;
   else
    {
    label->frame=frame;
    label->module=module;
    ...
    else label->offset=SIZ(x);
    if(gidx) label->group=grouparray[gidx-1];
    }
   x+=2;
   IDXINC(x);
   }
 ...
 case 0xB0:			/* Common variable */
  while(x!=nxt)
   ...
   if(pgm[x++]==0x61)
    {
    unsigned long size;
    if(!pgm[x&0x80]) size=pgm[x++];
    else switch(pgm[x++])
     {
    case 0x81: size=pgm[x]+(pgm[x+1]<<8); x+=2; break;
    case 0x84: size=pgm[x]+(pgm[x+1]<<8)+(pgm[x+2]<<16); x+=3; break;
    case 0x88: size=pgm[x]+(pgm[x+1]<<8)+(pgm[x+2]<<16)+(pgm[x+3]<<24); x+=4; break;
     }
    ...
    if(!pgm[x&0x80]) amnt=pgm[x++];
    else switch(pgm[x++])
     {
     case 0x81: amnt=pgm[x]+(pgm[x+1]<<8); x+=2; break;
     case 0x84: amnt=pgm[x]+(pgm[x+1]<<8)+(pgm[x+2]<<16);  x+=3; break;
     case 0x88: amnt=pgm[x]+(pgm[x+1]<<8)+(pgm[x+2]<<16)+(pgm[x+3]<<24); x+=4; break;
     }
    size*=amnt;
 ...
 default:
  fprintf(stderr,
   "Unknown record type %2.2X in object module \'%s\'\n",d,module->name);
  errval=1;
  }
 goto loop;
 }
...
void dopass2(FILE *fd,struct module *module)
 ...
 case 0x9c:			/* Fixup */
  ...
  while(x!=nxt)
   ...
   if(!(c&128))
    {
    int idx=(c&3)+(4&(c>>4));
    threads[idx].method=(7&(c>>2));
    if(c&64)
     switch(threads[idx].method)
      {
     case 0: threads[idx].seg=segarray[IDXINC(x)-1]; break;
     case 1: threads[idx].seg=grouparray[IDXINC(x)-1]; break;
     case 2: threads[idx].label=externarray[IDXINC(x)-1]; break;
      }
    else
     switch(threads[idx].method&3)
      {
     case 0: threads[idx].seg=segarray[IDXINC(x)-1]; break;
     case 1: threads[idx].seg=grouparray[IDXINC(x)-1]; break;
     case 2: threads[idx].label=externarray[IDXINC(x)-1]; break;
    ...
    {
    int locat=pgm[x++]+((c&3)<<8);	/* Offset from 'last' */
    int rel=!(c&64);			/* Set if branch fix */
    int what=(7&(c>>2));		/* What is being fixed */
    int fmethod;
    struct segment *fseg;
    struct label *flabel;
    int tmethod;
    ...
    if(c&128)
     ...
     switch(fmethod)
      {
     case 0: fseg=segarray[IDXINC(x)-1]; break;
     case 1: fseg=grouparray[IDXINC(x)-1]; break;
     case 2: flabel=externarray[IDXINC(x)-1]; break;
      }
     }
    if(c&8)
     {
     int idx=c&3;
     tmethod=(~4&threads[idx].method)+(c&4);
     ...
     switch(tmethod&3)
      {
     case 0: tseg=segarray[IDXINC(x)-1]; break;
     case 1: tseg=grouparray[IDXINC(x)-1]; break;
     case 2: tlabel=externarray[IDXINC(x)-1]; break;
      }
     }
    if(tmethod>=4) tmethod-=4, disp=0;
    else disp=SIZINC(x);
    switch(fmethod)
     {
    case 4: fmethod=0;
    ...
    switch(tmethod)
     {
    case 0:
     seg=(tseg->addr>>4); off=tseg->pos; break;
    case 1:
     seg=(tseg->gaddr>>4); off=tseg->pos; break;
    case 2:
     if(tlabel->seg!=NULL)
             {
    ...
    switch(fmethod)
     {
    case 0: seg1=(fseg->addr>>4); off1=0; break;
    case 1: seg1=(fseg->gaddr>>4); off1=0; break;
    case 2: if(flabel->seg!=NULL)
             {
             if(flabel->group!=NULL)
              {
              seg1=(flabel->group->gaddr>>4);
              off1=flabel->offset;
              }
             else
              {
              seg1=(flabel->seg->addr>>4);
              off1=flabel->offset;
              }
             }
            else
    ...
    /* Make seg/off relative to seg1/off1 */
    /* Oh never mind.  Segment always guarenteed to begin on
     * paragraph bounderies.  off1==0 always */
    off+=(seg-seg1)*16;
    /* Add in displacement */
    off+=disp;
    seg=seg1;
    ...
    if(rel)
     ...
     off-=locat+lastptr-last->buf+last->start;
     }

    /* Add fixup object to seg/off.  Tasm stores offsets in the image,
    ...
    /* Fix an offset */
    case 5:
    case 1: lastptr[locat+1]=(off>>8);
    
    /* Fix only low byte of an offset */
    case 0: lastptr[locat]=off;
            break;

    /* Fix a segment */
    case 2: lastptr[locat]=seg;
            lastptr[locat+1]=(seg>>8);
            break;

    /* Fix a pointer */
    case 3: lastptr[locat]=off;
            lastptr[locat+1]=(off>>8);
            lastptr[locat+2]=seg;
            lastptr[locat+3]=(seg>>8);
            break;

    default: fprintf(stderr,"Unknown fixup object\n"), errval=1;
     }
    }
   }
  }
 ...
 case 0x98:			/* Segment definition */
  ...
  seg=namearray[nidx-1];
  if(!combine) seg=private1find(seg->name,module);
  segarray[segidx++]=seg;
  ...
  if(seg->combine!=6)
   ...
   if(len)
    if(!seg->inc)
     switch(align)
      {
     case 2: seg->pos=((seg->pos+1)&~1); break;
     case 3: seg->pos=((seg->pos+15)&~15); break;
     case 4: seg->pos=((seg->pos+255)&~255); break;
      }
    else
     switch(align)
      { /* I think this can never happen */
        /* Except maybe with common blocks */
     case 2: seg->inc=((seg->inc+1)&~1); break;
     case 3: seg->inc=((seg->inc+15)&~15); break;
     case 4: seg->inc=((seg->inc+255)&~255); break;
      }
   seg->inc+=len;
   }
  else seg->contribo[seg->contribp-1]=0;
  }
  break;
 ...
 for(x=0;x!=NSEGMENTS;++x)
  for(seg=segments[x];seg;seg=seg->next)
   if(seg->module!=NULL)
    {
    printf(" %s",seg->name);
    if(!seg->combine) printf("(%s)",seg->module->name);
    }
 printf("\n");
  ...
  for(seg=segments[x];seg;seg=seg->next)
   if(seg->clst!=NULL)
    ...
    for(z=0;z!=seg->csiz;++z)
     ...
     if(!seg->clst[z]->combine) printf("(%s)",seg->clst[z]->module->name);
     if(z!=seg->csiz-1) printf(",");
     }
    printf(")");
    }
 printf("\n\n");
 ...
 if(lowseg!=NULL)
  ...
  if(lowseg!=NULL)
   ...
   if(naddr<addr)
    ...
    if(naddr!=addr) fwrite(zeros,naddr-addr,1,fd);
    addr=naddr;
    fwrite(lowseg->buf,lowseg->end-lowseg->start,1,fd);
...
unsigned long bind(struct segment *seg,unsigned long addr,unsigned long ld)
 ...
 if(seg->set)
  ...
  {
  if(seg->alignment==4) addr=((addr+255)&~255);
  else addr=((addr+15)&~15);
  seg->addr=addr;
  seg->ldofst=ld;
  addr+=seg->length;
  seg->set=1;
...
void position(void)
 ...
 for(x=0;x!=NSEGMENTS;++x)
  for(seg=segments[x];seg;seg=seg->next)
   if(seg->module!=NULL)
    {
    if(seg->alignment==0) bind(seg,(unsigned long)seg->frame<<4,0L);
 ...
 if(x!=y)
  ...
  if(seg->clst!=NULL)
   {
   int z;
   for(z=0;z!=seg->csiz;++z) addr=bind(seg->clst[z],addr,ld);
   }
  else
   {
   if(seg->module==NULL) seg=privatefind(buf+x);
   if(seg==NULL) printf("No such segment \'%s\'\n",buf+x);
   else addr=bind(seg,addr,ld);
   }
  buf[y]=c;
  goto loop;
  }
 ...
 for(x=0;x!=NSEGMENTS;++x)
  for(seg=segments[x];seg;seg=seg->next)
   if(seg->module!=NULL && !seg->set) goto loop1;
 }
  ...
  for(seg=segments[x];seg;seg=seg->next)
   if(seg->gmodule!=NULL)
    ...
    for(y=0;y!=seg->lstsiz;++y)
     {
     if(seg->seglst[y]->addr<lowest) lowest=seg->seglst[y]->addr;
...
void show(char *listfile)
 ...
 for(x=0;x!=NSEGMENTS;++x)
  for(seg=segments[x];seg;seg=seg->next)
   if(seg->module!=NULL)
    {
    int z;
    if(seg->start<seg->end) fprintf(fd,"%-23s %7.5lX %4lX (%lX-%lX)\n",seg->name,seg->addr,seg->length,seg->start,seg->end-1);
    else fprintf(fd,"%-23s %7.5lX %4lX\n",seg->name,seg->addr,seg->length);
  ...
  for(seg=segments[x];seg;seg=seg->next)
   if(seg->seglst!=NULL)
    ...
    for(z=0;z!=seg->lstsiz;++z)
     ...
     if(!seg->seglst[z]->combine) fprintf(fd,"(%s)",seg->seglst[z]->module->name);
     }
    fprintf(fd,"\n");
    }
 ...
 if(nlabels)
  ...
  for(x=0,y=0;x!=NLABELS;++x) for(l=labels[x];l;l=l->next) lbls[y++]=l;
  ...
  for(x=0;x!=nlabels;++x)
   {
   l=lbls[x];
   if(l->module!=NULL) sprintf(sbuf,"%s(%s)",l->name,l->module->name);
   else sprintf(sbuf,"%s(UNDEFINED)",l->name);
   fprintf(fd,"%-23s %4lX",sbuf,l->offset);
   if(l->seg!=NULL)
    if(!l->seg->combine) fprintf(fd," %s(%s)",l->seg->name,l->seg->module->name);
    else fprintf(fd," %7s",l->seg->name);
   else fprintf(fd," %7lX",l->frame);
   for(y=0;y!=l->nref;++y) fprintf(fd," %s",l->ref[y]->name);
   fprintf(fd,"\n");
   }
  }
 fclose(fd);
 ...
 for(y=0;y!=l->size;y+=512)
  ...
  for(h=0;h!=37;++h)
   ...
   if(q)
    ...
    ncpy(buf,dict+y+q);
    hval=hash(buf);
    e=(struct entry *)aalloc(sizeof(struct entry));
    e->name=strdup(buf);
  ...
  default:
    ...
    if(c!= -1)
     ...
     for(len=0;
         ...
         c=getc(fd))
      {
      if(len==bksiz-1) name=(char *)aealloc(name,bksiz+=13);
      name[len++]=c;
      }
     name[len]=0;
     dofile(name);
...
int main(int argc,char *argv[])
 ...
 if(argc<2)
  fprintf(stderr,"\
 Absolute linker for generating simple binary image files - Version 1.0\n\
 Written by Joseph H. Allen\n\
 \n\
 Syntax: alink [-q] [-l listfile] [-o outputfile] files\n\
 \n\
 ...
 "), errval=1;
  ...
  int x;
  for(x=1;argv[x];++x)
   if(!strcmp(argv[x],"-q")) quiet=1;
   else if(!strcmp(argv[x],"-l"))
    if(argv[x+1]==NULL) fprintf(stderr,"Missing listing file name\n"), exit(1);
    else listfile=argv[++x];
   else if(!strcmp(argv[x],"-o"))
    if(argv[x+1]==NULL) fprintf(stderr,"Missing output file name\n"), exit(1);
    else outfile=argv[++x];
   else dofile(argv[x]);		/* First pass process file */
  resolve();			/* Link in necessary library object modules */
  if(!quiet) showsegs();		/* List segments for user */
  position();			/* Get segment address and ordering info. */
  groups();			/* Determine group addresses */
  pass2();			/* Load and Fixup object modules */
  if(listfile!=NULL) show(listfile);	/* Generate listing file */
  if(outfile!=NULL) emit(outfile);	/* Emit output file */
  }
 return errval;
 }
