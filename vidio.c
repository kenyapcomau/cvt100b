...
#ifndef	NODOS
static char id[] = "$Id: vidio.c,v 1.1 1998/06/24 13:07:06 ken Exp $";
#endif
...
#ifdef	__BCC__
		movedata(0x40, 0x87, __get_ds, &b, 1);
					/* Check BIOS data to see if the */
		if ((b & 0x8) == 0)	/* EGA is the active adapter */
#else
...
#ifdef	NODOS
    vtprintf(0,columns-strlen(idstring),1,idstring);
#endif
...
#else
#ifdef	__TINY__
       if ( (screen_save = MK_FP(_DS, (unsigned)malloc( lines * columns * 2 ))) == NULL)
#else
